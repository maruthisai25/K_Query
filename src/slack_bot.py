...
class SlackBot:
    def __init__(self):
        self.config = Config()
        self.app = AsyncApp(
            token=self.config.SLACK_BOT_TOKEN, signing_secret=self.config.SLACK_SIGNING_SECRET
        )
        ...
        try:
            ...
            if not user_query:
                await respond(
                    ...
                    "Example: `/devops why are my pods crashing?`"
                )
                return
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(self.chat_api_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        return {
                            "response": "Sorry, I'm having trouble processing "
    ...
    def _format_slack_response(self, response: Dict) -> Dict:
        ...
        blocks = [
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"⚙️ *DevOps Automation*\n\n{response_text}"},
            }
        ]
