...
class K8sClient:
    ...
    async def get_relevant_info(self, query: str) -> str:
        ...
        try:
            ...
            return (
                " | ".join(info_parts)
                if info_parts
                else "No relevant Kubernetes information found."
            )
    ...
    async def _get_pods_info(self) -> str:
        ...
        try:
            ...
            pods = await loop.run_in_executor(
                None, lambda: self.v1.list_namespaced_pod(namespace=self.namespace)
            )
    ...
    async def _get_deployments_info(self) -> str:
        ...
        try:
            ...
            deployments = await loop.run_in_executor(
                None, lambda: self.apps_v1.list_namespaced_deployment(namespace=self.namespace)
            )
    ...
    async def _get_services_info(self) -> str:
        ...
        try:
            ...
            services = await loop.run_in_executor(
                None, lambda: self.v1.list_namespaced_service(namespace=self.namespace)
            )
    ...
    async def _get_recent_events(self) -> str:
        ...
        try:
            ...
            events = await loop.run_in_executor(
                None, lambda: self.v1.list_namespaced_event(namespace=self.namespace)
            )

            # Sort by timestamp and get recent events
            recent_events = sorted(
                events.items, key=lambda x: x.last_timestamp or x.first_timestamp, reverse=True
            )[:3]
    ...
    async def health_check(self) -> bool:
        ...
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, lambda: self.v1.list_namespace(limit=1))
            return True
        except Exception as e:
