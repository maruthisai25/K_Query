...
class VectorStore:
    ...
    async def _populate_initial_faqs(self):
        ...
        # These are documents, not code. Left on one line each so the stored
        # text is exactly what is written here; hence the E501 exemptions.
        faqs = [
            {
                "content": "To troubleshoot pod issues: 1) Check pod status with kubectl get pods -n <namespace> 2) Describe the pod with kubectl describe pod <pod-name> 3) Check logs with kubectl logs <pod-name> 4) Common issues include ImagePullBackOff (wrong image), CrashLoopBackOff (app crashes), and Pending (resource constraints)",  # noqa: E501
                "source": "kubernetes-troubleshooting.md",
                "category": "kubernetes",
                "tags": ["pods", "troubleshooting", "kubectl"],
            },
            {
                "content": "High CPU usage troubleshooting: 1) Check resource limits with kubectl describe pod <pod-name> 2) Monitor with kubectl top pods 3) Common causes: insufficient CPU limits, inefficient code, memory leaks causing CPU thrashing, high traffic load 4) Solutions: increase CPU limits, optimize application code, implement horizontal pod autoscaling",  # noqa: E501
