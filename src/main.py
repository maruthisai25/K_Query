...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if any(
            ...
            for keyword in ["pod", "deployment", "service", "namespace", "kubectl"]
        ):
        ...
        if any(
            keyword in request.message.lower()
            for keyword in ["metrics", "cpu", "memory", "error", "latency"]
        ):
        ...
        response = await llm_service.generate_response(
            message=request.message, context=full_context.strip()
        )
        ...
        raise HTTPException(
            status_code=503, detail="The model service is unavailable. Please try again shortly."
        )
    except Exception as e:
        duration = time.time() - start_time
        CHAT_DURATION.observe(duration)
